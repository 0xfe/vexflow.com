...
function minuet1() {
  ...
  vf.getContext().scale(width / 1100, width / 980);
  ...
  let x = 120;
  let y = 80;
  function makeSystem(swidth) {
    const system = vf.System({
      x, y, width: swidth, spaceBetweenStaves: 10,
    });
    x += swidth;
    return system;
  }

  function id(vid) { return registry.getElementById(vid); }
  ...
  system.addStave({
    voices: [
      ...
      voice([vf.TextDynamics({
        text: 'p', duration: 'h', dots: 1, line: 9,
      })]),
    ],
  })
    .addClef('treble')
    ...
    .setTempo({
      name: 'Allegretto', duration: 'h', dots: 1, bpm: 66,
    }, -30);
  ...
  const grace = vf.GraceNote({
    keys: ['d/3'], clef: 'bass', duration: '8', slash: true,
  });
  ...
  system.addStave({
    voices: [
      ...
      voice([vf.TextDynamics({
        text: 'mf', duration: 'h', dots: 1, line: 10,
      })]),
    ],
